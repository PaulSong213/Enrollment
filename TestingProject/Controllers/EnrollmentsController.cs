...
namespace EnrollmentSystem.Controllers
{
    public class EnrollmentsController : Controller
    {
        ...
        public ActionResult Enrolled()
        {
            ...
            con.Open();
            com.Connection = con;
            com.CommandText = $"SELECT * FROM [dbo].[enrollments] AS enrollments INNER JOIN (select id, name as courseName, acronym as courseAcronym from courses) [courses] ON [enrollments].[courseId] = [courses].[id] INNER JOIN (select id, birthDate as studentBirthDate, firstName as studentFirstName, middleName as studentMiddleName, lastName as studentLastName, gender as studentGender, age as studentAge, address as studentAddress, contactNumber as studentContactNumber, email as studentEmail, profileFileName as studentProfileFileName from students)  [students] ON [enrollments].[studentId] = students.id WHERE  status = 'accepted'";
            dr = com.ExecuteReader();
            if (dr.HasRows)
            {
                while (dr.Read())
        ...
        public ActionResult Accept(FormCollection collection)
        {
            ...
            con.Open();
            com.Connection = con;
            com.CommandText = $"SELECT link FROM [dbo].[schedules] WHERE year = {collection["year"]} AND section = {collection["section"]}";
            dr = com.ExecuteReader();
            ...
            var htmlString = getEmailString(true, collection, scheduleLink);
            try
            {
        ...
        public string getEmailString(Boolean isAccepted, FormCollection collection, string classScheaduleLink)
        {
